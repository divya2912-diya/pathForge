...
export function TopBar({ title, active, onHomeClick, onProfileClick, student }) {
  return (
    <div
      className="flex items-center justify-between pl-36 sm:pl-40 pr-5 md:pr-8 py-4 sm:py-5 sticky top-0 z-20"
      style={{ background: "rgba(6,9,17,0.7)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}
    >
      <div className="flex items-center gap-3 min-w-0">
        <h1 className="lp-display text-lg font-semibold truncate text-white">{title}</h1>
      </div>
